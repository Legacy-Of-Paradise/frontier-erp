## Entity

microwave-component-range-interact-item-too-big = { CAPITALIZE(THE($item)) } слишком большое, чтобы запихнуть в духовку!
microwave-component-assembler-interact-item-too-big = { CAPITALIZE(THE($item)) } слишком большое, чтобы запихнуть в едамат!
microwave-component-medical-assembler-interact-item-too-big = { CAPITALIZE(THE($item)) } слишком большое, чтобы запихнуть в медсборник!
microwave-component-upgrade-cook-time = время готовки
microwave-component-assembler-no-valid-recipe = Не найден подходящий рецепт!

## Bound UI

assembler-bound-user-interface-insert-ingredients = ВСТАВЬТЕ МАТЕРИАЛЫ
assembler-bound-user-interface-cook-time-label = ВРЕМЕНИ ОСТАЛОСЬ: { $time }

## UI

microwave-menu-range-title = Электроплита
microwave-menu-range-footer-flavor-left = NanoTrasen не несет ответственности за ущерб, возникший в результате пожара на кухне.
assembler-menu-title = Едамат
assembler-menu-medical-title = Медсборник
assembler-menu-start-button = Сборка
assembler-menu-footer-flavor-left = Не забывайте хранить продукты при безопасной температуре.
assembler-menu-medical-footer-flavor-left = Сертифицированная наностерильность 99,99%.
assembler-menu-footer-flavor-right = v0.2.1
